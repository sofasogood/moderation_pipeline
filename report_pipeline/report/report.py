...
from report_pipeline.categories.categories import Categories 
import pandas as pd
from typing import List, Tuple, Union, Callable
from tabulate import tabulate
REPORT_VERSION="v1.0" # Initial report version
CLASSIFIER_VERSION="v1" # Initial classifier version
class ReportGenerator:
  ...
  def generate_executive_summary(self, data: pd.DataFrame, summary_stats: pd.DataFrame, prev_day_data: pd.DataFrame = None) -> str:
    ...
    category_cols = [x for x in Categories().as_keys() if x!='N']
    ...
    category_names = Categories().as_dict()
    summary.extend([
        "\nTOP CATEGORIES:",
  ...
  def analyze_critical_patterns(self, data: pd.DataFrame, 
      ...
      for category in sorted(Categories().as_keys()):
          if category == 'N':
              continue
  ...
  def generate_report(self,     
        ...
        if sections is None:
          ...
          category_cols = [col for col in data.columns if col in Categories().as_keys()]  # Add other category names as needed
